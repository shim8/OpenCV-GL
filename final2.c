...
void display(void)
{
  ...
  for (i = 0; i < (faces ? faces->total : 0); i++) {
  glLoadIdentity();
  glTranslatef(7.0-14.0*((double)center[i].x/img->width), 7.0+27.0*((double)radius[i]/img->height)-14.0*((double)center[i].y/img->height) + f, -10.0);
  glRotatef(90.0, 1.0, 0.0, 0.0);
  color();
  glPushMatrix();
  glutSolidTorus(2.0*((double)radius[i]/img->width), 20.0*((double)radius[i]/img->width), 100, 100);
  glPopMatrix();
  }
